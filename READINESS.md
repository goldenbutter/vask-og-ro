...
|---|---|---|---|
| Payment modal phrase — Stripe (booking-led niche), all words verbatim | ✅ | ✅ | ⚠️ **punctuation deviates from family-locked phrase:** em-dash → period per Bithun's zero-em-dash directive (pass #2 FIX 4). Now `Dette er en demo. Stripe-betaling aktiveres når din Stripe-konto er koblet til.` (every word preserved). Family-wide propagation of the locked phrase is an architect concern. |
...
| Equipment showcase ("Vårt utstyr", om-oss) — 8 photos in a clean 4×2 grid, card-float + `.is-tapped` iOS twin (static cream cards on classic) | ✅ |
| One-shot nav-logo hover rotate; scroll-reveal; ring-pulse phone CTA | ✅ |
| Home trust-strip left-to-right marquee (`trust-marquee`, hover-pause, reduced-motion → static; classic static single strip) | ✅ |
...
- 2026-06-13 18:10 norsk sommertid — S58 DEVELOPER post-build fix pass #2 (Bithun smoke-walk round 2; 5 atomic commits, both tiers, local `main`, no push). **FIX 4** — stripped ALL em-dashes (U+2014, ~250 across HTML + `js/main.js` both tiers) from visible copy + comments; sense-based (comma/period/colon/middot), en-dash ranges (`Man–fre`, `2–4 timer`) preserved; gate `grep -rl $'—' …` = empty. ⚠️ touched the family-locked Stripe phrase punctuation (em-dash→period, all words preserved — see §1 row). **FIX 1** — added 8th equipment card (`equipment-cart.jpg` "Rengjøringstralle"/`Cleaning trolley`) → clean 4×2 grid both tiers (`equip.cart` key NO+EN). **FIX 2** — `.contact-grid` `1.1fr .9fr`→`1fr 1fr` (balanced) + `.form-submit` `width:100%`→`auto` inline button. **FIX 3** — deposit: muted confirm caption "Depositum 500 kr, bekreftes ved betaling" (`deposit.confirm` NO+EN) above a content-width `Betal`/`Pay` button (`.deposit-cta width:auto; align-self:flex-start`; `.stripe-trigger` kept). **FIX 5** — home trust strip → left-to-right `translateX` marquee (premium scrolls, hover-pause + reduced-motion guard; classic static single strip via the override tail; rule #31). Headless-verified @375 + @1280 both tiers (chrome-headless-shell — MCP browser held by a parallel session). Commits `f7ebf06`/`c239f0d`/`b962923`/`c277b22`/`954cb81`. Reviewer re-spawn by on-stop hook; nothing pushed.

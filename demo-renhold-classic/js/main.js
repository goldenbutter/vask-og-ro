/* ============================================================
   VASK & RO - CLASSIC - main.js
   Vanilla, dependency-free. NO-only baseline: the i18n block is
   comment-wrapped (strings preserved for the paid bilingual
   upsell) and the motion inits are disabled in DOMContentLoaded.
   The static-override tail in style.css strips the visuals.
   ============================================================ */
(function () {

  var REDUCE = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  var INSTANT = true; /* classic: total updates instantly, no count-up */

  /* =========================================================
     i18n DISABLED on classic - Norwegian-only baseline.
     Strings preserved for the paid bilingual upsell. To revive:
       1. remove the opening and closing block-comment markers that
          wrap this block (the line just below, and the matching one
          just before initNav).
       2. uncomment the EN .lang-toggle in nav + mobile menu (all pages)
       3. uncomment applyTranslations() + initLangToggle() in DOMContentLoaded
     ========================================================= */
  /*
  var translations = {
    no: {
  ...
  */
  /* end i18n wrap - classic is Norwegian-only */
  ...
  function initBeforeAfter() {
    document.querySelectorAll('.ba-frame').forEach(function (frame) {
      ...
      function setTag(on) { if (tag) tag.textContent = on ? after : before; }
  ...
  function initCardTapHover() {
    var selector = '.cards .card, .service-list .service-row';
  ...
  document.addEventListener('DOMContentLoaded', function () {
    // applyTranslations();   // disabled - i18n block wrapped above (NO-only baseline)
    // initLangToggle();      // disabled - EN toggle is commented out in the HTML
    initNav();
    // initScrollReveal();    // disabled - classic content is statically visible
    // initTypewriter();      // disabled - hero shows the first phrase statically
    // initFixedBackdrop();   // disabled - atmospheric backdrop is premium-only
    initCalculator();
    // initBeforeAfter();     // disabled - classic uses the static side-by-side grid
    // initCardTapHover();    // disabled - card-float/tap is premium-only
    initStripeCheckout();
    initContactForm();
    // main page-fade disabled - classic has no entrance animation
  });
